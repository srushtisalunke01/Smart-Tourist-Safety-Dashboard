...
async function getCurrentSchemaVersion(client) {
    var _a, _b;
    const { rows } = await client.query('SELECT COALESCE(MAX(version), 0)::int AS version FROM bullmq_migration');
    return (_b = (_a = rows[0]) === null || _a === void 0 ? void 0 : _a.version) !== null && _b !== void 0 ? _b : 0;
}
async function ensureLedgerTable(client) {
    await client.query(`CREATE TABLE IF NOT EXISTS bullmq_migration (
...
async function applyMigration(client, migration) {
    ...
    await client.query('INSERT INTO bullmq_migration (version, name) VALUES ($1, $2)', [migration.version, migration.name]);
}
